...
- **D-33 (2026-08-06):** Resolves 03-08's flagged open requirement. D-31's live-location foreground service MUST also survive the sender fully force-killing/swiping away the app during an active SOS window, not just backgrounding/locking. Configure the Android service declaration with `android:stopWithTask="false"` plus a self-restart mechanism (e.g. `onTaskRemoved` re-launching the foreground service, or the equivalent the installed `flutter_foreground_task` version exposes) so task removal does not silently end the stream mid-emergency. Rationale: the core value is that SOS must always work, and an attacker or a panicking user swiping the app away is exactly the scenario a safety app needs to resist — the *initial* alert + last-known location already reaches guardians synchronously through the trigger path (SOS-01/02) regardless of this decision, so what's at stake here is only the *ongoing* live-tracking feed. Explicitly accepted as a residual, undocumented-as-solvable risk: aggressive OEM battery managers (Xiaomi, Huawei, Samsung and similar) can still kill background services regardless of `stopWithTask`/manifest configuration — no app can fully close that gap, and 03-08's SUMMARY must document it as a known platform limitation rather than imply it's solved.
