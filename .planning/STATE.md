...
| Migrate SOS fallback channel from TextBee to WhatsApp Business Cloud API — decision locked, blocked on Meta approving the user's submitted Utility-category SOS alert template; resume via /gsd-quick once approved | backend | [todos/pending/2026-08-10-evaluate-an-alternate-sms-provider-besides-textbee.md](./todos/pending/2026-08-10-evaluate-an-alternate-sms-provider-besides-textbee.md) |
...
| Account verification email arrives with an empty body (no link, no content) — has always been broken; channel/provider not yet identified. Live debug session at `.planning/debug/verification-email-empty.md` | backend/auth | [todos/pending/2026-08-11-auth-verification-email-arrives-empty.md](./todos/pending/2026-08-11-auth-verification-email-arrives-empty.md) |
