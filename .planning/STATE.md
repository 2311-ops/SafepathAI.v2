...
last_activity_desc: "Completed quick task 260814-j70: Apply Home Map screen redesign (status pill, member pin rings/pulse, draggable bottom sheet)"
progress:
  total_phases: 8
  completed_phases: 4
  total_plans: 61
...
Last activity: 2026-08-14 - Completed quick task 260814-j70: Apply Home Map screen redesign (status pill, member pin rings/pulse, draggable bottom sheet)
...
| 260814-amy | Rebranded the locked SafePath design system per user decision (superseding the original 36-screen palette): all 25 AppColors members recolored to navy #1B2A4A / SOS red #E53935 / background #F5F7FA / secondary text #6B7A99, accent green darkened from the source spec's #00C896 to #00875F to clear WCAG AA 4.5:1 text/icon contrast (user-resolved checkpoint decision, applied uniformly — no separate fill-only value exists), all 10 AppTypography roles moved from Manrope/JetBrains Mono to Inter via google_fonts, new AppRadius (16/12/24px card/button/bottom-sheet) wired through app_theme.dart including a new bottomSheetTheme, theme_test.dart assertions updated, and .claude/CLAUDE.md + .planning/PROJECT.md's Design fidelity bullet rewritten to document the new system as now-locked. Screen-level structural/animation changes from the broader UI spec (bottom sheets, hold-to-activate SOS, skeleton loaders, dark mode, etc.) are explicitly out of scope, follow-up quick tasks. Post-execution verification found and this pass corrected one doc-accuracy gap (both docs wrongly implied a separate full-strength fill-only accent value existed) | 2026-08-14 | 4e658e2, ef4bc50, 6340a4e, 46e5267 | Verified | [260814-amy-replace-locked-design-system-with-new-pa](./quick/260814-amy-replace-locked-design-system-with-new-pa/) |
| 260814-j70 | Applied the Home Map redesign spec. Investigation found SOS button (72px/deep-red/hold-to-activate/3s ring), the four-tab bottom nav, full-screen map, and muted OpenFreeMap tile style already matched spec exactly — untouched. Split live_map_screen.dart's single _LiveMapOverlay header into a slim _MapTopBar (identity pin + lowercase "N online · N offline" _FamilyStatusPill + profile + logout) and a new DraggableScrollableSheet (0.2->0.5) housing Manage-safe-zones/Notifications; converted LiveMemberMarker to a StatefulWidget with a presence-colored ring (green online / grey offline) and a self-only animated pulse ring, respecting reduce-motion; added kShellBottomBarHeight to main_shell.dart as the sheet's sole touch point there (SOS arm/hold/trigger logic, sosControllerProvider calls, and the four _tabs confirmed byte-identical by independent post-execution diff). Extended widget test coverage for the new pill/sheet/rings and fixed a dead test sentinel string | 2026-08-14 | fb539ab, 4b05288, f9692f6 | Verified | [260814-j70-apply-home-map-screen-redesign-status-pi](./quick/260814-j70-apply-home-map-screen-redesign-status-pi/) |
