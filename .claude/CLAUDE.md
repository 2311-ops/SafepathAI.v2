...
**Core Value:** The SOS system must always work: a single tap on the visible SOS button, or a covert
...
- **Tech stack**: Flutter/Dart (mobile, OpenFreeMap Liberty vector tiles via maplibre_gl, FCM), ASP.NET Core Web API + SignalR
...
|---------|---------|---------|-------------|
| `maplibre_gl` | **0.26.2** | Live map, geofence visualization, history/route rendering | OpenFreeMap serves vector tiles only (no raster endpoint), so a vector-capable native renderer is required to use it at all (project direction changed 2026-08-06 from the raster `flutter_map`/OSM-tile stack). Renders OpenFreeMap's **Liberty** style flat, with tilt gestures disabled so its building-extrusion layer can never render in 3D (D-01). The public OpenFreeMap instance is used as-is: no API key, no usage limits (D-02; self-hosting explicitly deferred). Attribution comes from the renderer's own bottom-right attribution control, fed by the source TileJSON, covering OpenFreeMap/OpenMapTiles/OpenStreetMap. See `.planning/phases/02-real-time-location-history-privacy/02-OSM-MIGRATION-IMPACT.md` for the superseded raster-tile retrofit history and `.planning/quick/260806-3zb-migrate-map-rendering-from-flutter-map-t/` for the vector-renderer migration itself. |
...
| `maplibre_gl` (native vector renderer) | No native marker-clustering package | Marker pins (family members, route stops) are Flutter widgets in a screen-space overlay reprojected via the native projection (`toScreenLocationBatch`), not native symbol annotations — so no native clustering package applies at MVP scale. Revisit only once family/guardian overview maps show enough simultaneous markers to need clustering. |
