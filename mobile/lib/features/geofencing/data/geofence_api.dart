...
class DioGeofenceApi implements GeofenceApi {
  ...
  Future<SafeZone> create(SafeZoneDraft draft) async {
    ...
    if (familyId == null) throw const GeofenceApiException('Choose a family before saving.');
    try {
  ...
  SafeZone _fromMutation(
    ...
    id: response?['zoneId'] as String? ?? zoneId ?? draft.zoneId ?? '',
  ...
  GeofenceApiException _mapError(DioException error) {
    ...
    if (error.response?.statusCode == 403) {
      return const GeofenceApiException('Only Guardians can manage safe zones.');
    }
    if (error.response?.statusCode == 400 || error.response?.statusCode == 409) {
      return GeofenceApiException(serverMessage ?? 'That safe zone could not be saved.');
    }
    if (error.type == DioExceptionType.connectionError ||
        error.type == DioExceptionType.connectionTimeout ||
        error.type == DioExceptionType.receiveTimeout) {
      return const GeofenceApiException("Couldn't connect. Check your connection and try again.");
    }
    return GeofenceApiException(error.message ?? 'That safe zone could not be saved.');
  }
}
