...
void main() {
  test(
    'replaces the canonical set and drains only durable candidates',
    () async {
      ...
      expect(
        (await gateway.drainPendingCandidates()).single.eventId,
        'event-1',
      );
  ...
  test('uses one canonical request identity across native platforms', () {
    const zone = NativeGeofenceZone(
      zoneId: 'zone-1',
      generation: 7,
      latitude: 30.0444,
      longitude: 31.2357,
