...
void main() {
  ...
  testWidgets('slim top bar shows the aggregate family status pill', (
    tester,
  ) async {
    await tester.pumpWidget(
      ProviderScope(
        ...
        child: const MaterialApp(
          home: LiveMapScreen(mapPlatformViewBuilder: _fakePlatformViewBuilder),
        ),
      ),
    );
    // Never pumpAndSettle here: the self pin's pulse ring now animates
    // forever, so a fixed-duration pump is required (mirrors the other
    // populated-map tests above).
    await tester.pump(const Duration(milliseconds: 100));

    expect(find.text('1 online · 1 offline'), findsOneWidget);
  });

  testWidgets(
    'draggable action sheet holds Manage safe zones and Notifications, '
    'resting at 20% and opening to 50%',
    (tester) async {
      await tester.pumpWidget(
        ProviderScope(
          overrides: [
      ...
      expect(sheet.initialChildSize, 0.2);
      expect(sheet.maxChildSize, 0.5);
      expect(find.byType(ManageSafeZonesButton), findsOneWidget);
      expect(find.byType(ViewNotificationsButton), findsOneWidget);
    },
  );

  testWidgets(
    'a Member sees only Notifications in the action sheet, no Manage safe '
    'zones',
    (tester) async {
      await tester.pumpWidget(
        ProviderScope(
      ...
      expect(find.byType(ManageSafeZonesButton), findsNothing);
      expect(find.byType(ViewNotificationsButton), findsOneWidget);
    },
  );
}
